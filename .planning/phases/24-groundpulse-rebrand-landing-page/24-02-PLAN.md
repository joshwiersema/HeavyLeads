...
must_haves:
  truths:
    - "Landing page shows GroundPulse branding with GP monogram, not HeavyLeads"
    - "All 5 industries (heavy equipment, HVAC, roofing, solar, electrical) have dedicated showcase sections with specific lead examples"
    - "Stats section shows concrete numbers: 300+ city portals, 20+ data sources, 5 industries, 50 states"
    - "Interactive animated dashboard preview demonstrates the product visually"
    - "Copy uses construction industry language and specific examples, not generic AI phrases"
    - "Page has asymmetric visual personality with CSS micro-interactions"
    - "Hero, industries, how-it-works, stats, CTA, footer sections are present"
    - "No references to HeavyLeads or LeadForge anywhere on the page"
  artifacts:
    - path: "src/app/page.tsx"
      provides: "Complete landing page with 5-industry showcase and interactive demo"
      min_lines: 400
...
<objective>
Create a handcrafted landing page for GroundPulse that showcases all 5 industries with specific value propositions, concrete stats, and an interactive product demo element.

Purpose: The current landing page looks AI-generated and generic. The new page must feel like it was designed by someone who understands construction trades — using specific numbers, real lead examples, and trade-specific language. A first-time visitor should trust this page enough to start a trial.

Output: Complete rewrite of src/app/page.tsx with hero, industry showcase, how-it-works, stats/social-proof, CTA, and footer.
</objective>
...
<interfaces>
From src/app/page.tsx (current structure, will be fully replaced):
- Server component with auth check: redirects logged-in users to /dashboard or /onboarding
- Uses: auth from @/lib/auth, headers from next/headers, redirect from next/navigation
- Uses: db from @/lib/db, companyProfiles from @/lib/db/schema/company-profiles, eq from drizzle-orm
- Uses: Link from next/link, lucide-react icons
- Color scheme: bg-[#1a1a1e] charcoal, amber-400/500 gold accents, #8a8a95 muted text, #555560 dim text
</interfaces>
</context>
...
<task type="auto">
  <name>Task 1: Create the GroundPulse landing page with 5-industry showcase and interactive demo</name>
  <files>src/app/page.tsx</files>
  <read_first>src/app/page.tsx, src/app/layout.tsx, src/app/globals.css</read_first>
  <action>
    Completely rewrite `src/app/page.tsx` with the following content. Keep the server component auth-redirect logic at the top (lines 1-43 of current file), then replace everything in the return statement.

    **IMPORTANT design principles (from user decisions):**
    - Charcoal (#1a1a1e), white, gold (#c8a951 / amber-400-500) color scheme
    - Framer-inspired dark aesthetic
    - Asymmetric layout with visual personality (NOT a grid of identical cards)
    - CSS micro-interactions (hover states, subtle transitions) — no JS animation libs
    - Copy speaks to tradespeople, not corporate jargon
    - SPECIFIC numbers and industry examples throughout
    - NO generic AI phrases like "unlock the power of", "revolutionize", "cutting-edge", "game-changer", "leverage"
    ...
    Write this EXACT content to `src/app/page.tsx`. The component includes:

    1. **Nav**: GP monogram, GroundPulse text, Sign In + Start Free Trial buttons
    2. **Hero**: "Your next job is already on file somewhere." Copy about 300+ portals, 20+ sources. Two CTAs.
    3. **Interactive Dashboard Preview**: Mock browser chrome with fake sidebar, 3 realistic lead cards (HVAC permit, federal roofing contract, storm alert). This is the LAND-04 interactive/visual element — it demonstrates the actual product.
    4. **Industry Showcase (5 sections)**: Asymmetric grid (Heavy Equipment spans 2 cols, others 1 col each). Each has: icon with industry-specific color, trade subtitle, specific scenario copy, lead type tags.
    5. **How It Works**: 3 steps — scrape, score, show up first.
    6. **Data Sources Stats**: 4 stat cards (300+ portals, 20+ sources, 50 states, 5 industries) + source name badges.
    7. **Features**: 6-card grid with specific feature descriptions (scoring, storm alerts, pipeline, service area, digest, filters).
    8. **CTA**: "Stop Googling for permits." with trial button.
    9. **Footer**: GP monogram, GroundPulse, copyright, legal links.
  </action>
  <verify>
    <automated>cd C:/Users/joshu/Documents/HeavyLeadsApp && export PATH="/c/Program Files/nodejs:$PATH" && grep -c "GroundPulse" src/app/page.tsx && grep -c "HeavyLeads\|LeadForge" src/app/page.tsx && grep -c "industries" src/app/page.tsx</automated>
  </verify>
  <done>
    - Landing page has GroundPulse branding throughout (zero HeavyLeads/LeadForge references)
    - All 5 industries (Heavy Equipment, HVAC, Roofing, Solar, Electrical) have dedicated sections with trade-specific copy and examples
    - Stats section shows 300+ portals, 20+ sources, 50 states, 5 industries
    - Interactive dashboard preview mock demonstrates the product visually
    - Copy uses specific construction language (permits, filings, adjuster, conduit, ductwork) not generic AI phrases
    - Asymmetric layout (heavy equipment card spans 2 columns)
    - CSS hover transitions on cards and buttons
    - Auth redirect logic preserved for logged-in users
  </done>
</task>
...
1. `grep -c "GroundPulse" src/app/page.tsx` returns 5+ (brand name used throughout)
2. `grep -c "HeavyLeads\|LeadForge" src/app/page.tsx` returns 0
3. `grep "Heavy Equipment\|HVAC\|Roofing\|Solar\|Electrical" src/app/page.tsx` shows all 5 industries
4. `grep "300+\|20+\|50 States\|5.*Industries" src/app/page.tsx` shows concrete stats
5. `grep "dashboard" src/app/page.tsx` confirms interactive demo element exists
6. TypeScript compilation passes: `npx tsc --noEmit`
</verification>

<success_criteria>
- Landing page renders with GroundPulse branding
- All 5 industries showcased with specific, non-generic copy and trade-specific lead type tags
- Interactive dashboard preview shows mock lead cards with realistic data (scores, addresses, lead types)
- Concrete stats displayed (300+ portals, 20+ sources, 50 states, 5 industries)
- Copy speaks to tradespeople (mentions permits, filings, storms, adjusters, conduit, ductwork, iron on-site — not "revolutionize" or "unlock the power of")
- Asymmetric industry grid layout (not identical cards)
- CSS hover transitions on industry cards and CTA buttons
- No HeavyLeads or LeadForge references
- Auth redirect logic works (logged-in users still route to dashboard/onboarding)
</success_criteria>

<output>
After completion, create `.planning/phases/24-groundpulse-rebrand-landing-page/24-02-SUMMARY.md`
</output>
